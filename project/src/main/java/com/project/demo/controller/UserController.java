...
import com.project.demo.face.BaiDuAPIUtil;
import com.project.demo.face.Base64Util;
...
public class UserController extends BaseController<User, UserService> {
    ...
    public Map<String, Object> faceLogin(@RequestBody Map<String, String> data, HttpServletRequest httpServletRequest) {
        log.info("[执行登录接口]");
        String username = data.get("username");
        ...
        User byUsername = (User) resultList.get(0);
        //人脸识别
        String image1 = Base64Util.base64Encoding(byUsername.getAvatar());
        String faceImage = data.get("face_image");
        String image2 = Base64Util.base64Encoding(faceImage);

        Double score = BaiDuAPIUtil.matchPerson(image1,image2);
        if (score>=60){
            // 存储Token到数据库
            AccessToken accessToken = new AccessToken();
            accessToken.setToken(UUID.randomUUID().toString().replaceAll("-", ""));
